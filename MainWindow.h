...
namespace GlobalHooks {
	...
	protected:
		...
		static IntPtr MouseProc(int nCode, WPARAM wParam,					//при нажатии мыши, код хука
			LPARAM lParam)
		{
			...
			return (IntPtr)CallNextHookEx(mouseHookHandle, nCode, wParam, lParam);		//вызывает следующий хук
		}

		static IntPtr KeyboardProc(int nCode, WPARAM wParam,
			LPARAM lParam)
		{
			bool shiftState = false;						//нажатие Shift
			bool ctrlState = false;							//нажатие Ctrl
			bool winState = false;							//нажатие Win
			...
			if (GetAsyncKeyState(VK_LWIN) & (1 << 15))
				winState = true;
			//Alt
			if (wParam == WM_SYSKEYDOWN)					//если кнопка Alt была нажата
			{
			...
			//Alt + smth
			if (wParam == WM_SYSKEYUP)						//если Alt бы отпущен
			{
				string mes;
			...
			//Any other
			if (wParam == WM_KEYDOWN)						//если была наата любая другая кнопка
			{
				if (shiftState || ctrlState || winState)
				{
					...
					{
						if (instance->currentMode == MODE_SPY)				//если было скрытое
						{
							instance->DefaultMode();						//сделать видимым
							instance->currentMode = MODE_DEFAULT;			//запомнить что оно стало выидимым
						}
						else
						{
							instance->SpyMode();
			...
			if (remap != NO_REMAP)															//если нужно заменить
			{
				if (wParam == WM_KEYDOWN)													//если кнопка была нажата
					keybd_event(remap, 0x1D, KEYEVENTF_EXTENDEDKEY | 0, 0);
				else if (wParam == WM_KEYUP)												//если была отпущена
